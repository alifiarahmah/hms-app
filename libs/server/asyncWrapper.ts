...
const AsyncRoute = (fn: (req: any, res: any) => void) => {
  return function asyncUtilWrap(req: Request, res: Response, next: NextFunction) {
    const fnReturn = fn(req, res);
    return Promise.resolve(fnReturn).catch(next);
  };
};

const AsyncMiddleware = (fn: (req: any, res: any, next: NextFunction) => void) => {
  return function asyncUtilWrap(req: Request, res: Response, next: NextFunction) {
